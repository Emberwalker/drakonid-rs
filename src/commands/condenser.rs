...
impl CondenserShorten {
    pub fn new(client_data: &Arc<Mutex<ShareMap>>) -> Option<CondenserShorten> {
        ...
        let conf_res = || -> Result<_, ()> {
            ...
            let conf = conf_lock.get::<ConfigMarker>().expect("unable to load client config");
        ...
        if let Ok((key, server)) = conf_res() {
            ...
            opts.desc = Some(format!("Shorten a URL with the Condenser service at {}", server));
...
impl Command for CondenserShorten {
    fn execute(&self, _ctx: &mut Context, msg: &Message, mut args: Args) -> Result<(), CommandError> {
        let argc = args.len();
        if argc < 1 || argc > 2 {
            usage_error_embed("shorten", "Wrong number of arguments (must be 1 or 2)", Arc::clone(&self.opts), msg);
            return Ok(());
        }
        ...
        if argc == 1 {
            url = match args.single::<Url>() {
                Err(_) => {
                    usage_error_embed("shorten", "Unable to parse provided URL.", Arc::clone(&self.opts), msg);
                    return Ok(());
                },
                Ok(url) => url,
            };
            ...
            url = match args.find::<Url>() {
                Err(_) => {
                    usage_error_embed("shorten", "Unable to find a valid URL.", Arc::clone(&self.opts), msg);
                    return Ok(());
                },
                Ok(url) => url,
            };
        ...
        if url.scheme() != "http" && url.scheme() != "https" {
            usage_error_embed("shorten", &format!("Invalid URL scheme: {}", url.scheme()), Arc::clone(&self.opts), msg);
            return Ok(());
        }

        let srv_name = if let Some(guild) = msg.guild() { guild.read().name.clone() } else { "PM".into() };

        let request = ShortenRequest {
            url: url.into_string(),
            code,
            meta: Some(format!("Submitted via Drakonid by {} (via {})", msg.author.tag(), srv_name)),
        };
        ...
        run_on_worker(move || {
            ...
            let mut response = match response_result {
                ...
                Err(it) => match it.status() {
                    Some(code) => {
                        warn!("Invalid response but with a HTTP status code? {} -> {:?}", code, it);
                        error_embed(
                            channel_id,
            ...
            let parsed_response: Url = match response.status() {
                StatusCode::Ok => match response.json::<ShortenResponse>() {
                    ...
                    Err(err) => {
                        ...
                        error_embed(channel_id, "Unable to parse response from server.", Some(&usr_mention), |e| e);
                        return;
                    } // TODO
                },
                StatusCode::Conflict => {
                    error_embed(channel_id, "The provided code already exists.", Some(&usr_mention), |mut e| {
